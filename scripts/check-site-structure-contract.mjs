...
const EXPECTED_PAGES = {
  ...
  "episode-koko-feelings": {
    ...
    channel: "koko",
    localizedRoutes: { en: "/episodes/koko-feelings" },
  },
  "episode-noor-colors": {
    route: "/episodes/noor-colors",
    channel: "noor",
    localizedRoutes: { en: "/episodes/noor-colors" },
  },
};
...
async function validateHtmlRoutes(structure, context, failures, data) {
  ...
  for (const entry of expectedRouteEntries) {
    ...
    if (!js.includes("/js/site-shared-20260613-commerce2.js")) failures.push(`html_missing_shared_js:${entry.route}`);
...
async function validateLocalHtmlInventory(context, failures, data) {
  ...
  if (htmlFiles.length !== 16) failures.push(`html_file_count:${htmlFiles.length}`);
