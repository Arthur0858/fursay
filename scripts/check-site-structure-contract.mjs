...
async function validateHtmlRoutes(structure, context, failures, data) {
  ...
  for (const entry of expectedRouteEntries) {
    ...
    if (!js.includes("/js/site-shared-20260613-commerce5.js")) failures.push(`html_missing_shared_js:${entry.route}`);
...
async function validateLocalHtmlInventory(context, failures, data) {
  ...
  if (htmlFiles.length !== 30) failures.push(`html_file_count:${htmlFiles.length}`);
