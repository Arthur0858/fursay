...
const TOOL_ONLY_CSS = new Set([
  "/css/picture-world-tools-20260613-products1.css",
  "/css/sample-activity-20260718-v1.css",
]);
...
async function validateHtmlRoutes(structure, context, failures, data) {
  ...
  for (const entry of expectedRouteEntries) {
    ...
    if (!js.includes("/js/site-shared-20260615-sharekit1.js")) failures.push(`html_missing_shared_js:${entry.route}`);
...
async function validateLocalHtmlInventory(context, failures, data) {
  ...
  if (htmlFiles.length !== 94) failures.push(`html_file_count:${htmlFiles.length}`);
