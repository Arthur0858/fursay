...
const CSS = "/css/editorial-20260801-v1.css";
...
const TRUST = {
  about: {
    en: ["About Fursay", "Parent-led bilingual story moments", [
      ...
      ["What we make", "We publish story entry points, parent discussion prompts, episode guides, and short printable activities. Free samples come first. Paid checkout, public pricing, and payment links remain disabled while product, refund, support, and tracking requirements are reviewed."],
...
function articleBody(locale, guide) {
  const c = COPY[locale]; const d = guide.details[locale]; const h = c.headings;
  const originalParagraphs = new Set([d.scene, d.goal, d.routine, d.example, d.observe, d.limit, d.bridge]);
  const repeated = c.repeat;
  const reversible = locale === "zh" ? `有用的流程必須隨時能停止或改變。${d.bridge}` : locale === "ar" ? `يجب أن يبقى الروتين المفيد قابلًا للتوقف أو التغيير. ${d.bridge}` : `A useful routine remains reversible. ${d.bridge}`;
  const worldBridge = locale === "zh" ? (guide.world === "koko" ? "Koko 的森林故事提供畫面與短句，家庭真實對話則賦予意義。" : "努爾與 Zayd 提供畫面與中文短句，阿語家庭對話則提供理解與溫度。") : locale === "ar" ? (guide.world === "koko" ? "توفر قصص غابة Koko الصورة والعبارة، ويمنحها الحوار العائلي الحقيقي المعنى." : "توفر نور وZayd الصورة والعبارة الصينية، ويمنح الحوار العربي الفهم والدفء.") : (guide.world === "koko" ? "Koko's forest stories can supply the picture and phrase; the family's real conversation supplies the meaning." : "Nour and Zayd can supply the picture and Chinese phrase; Arabic family conversation supplies understanding and warmth.");
  const sections = [
    [h[0], [d.scene, repeated[0], `${c.methodLead} ${d.goal}`, repeated[6]]],
    [h[1], [d.goal, repeated[1], repeated[3]]],
    [h[2], [d.routine, repeated[4], reversible]],
    [h[3], [d.example, repeated[0], repeated[1]]],
    [h[4], [d.observe, repeated[2], repeated[3], repeated[7]]],
    [h[5], [d.bridge, repeated[4], worldBridge]],
    [h[6], [d.limit, repeated[5], c.intro]],
  ];
  return sections.map(([heading, paragraphs], index) => `<section id="section-${index + 1}" data-guide-section><h2>${esc(heading)}</h2>${paragraphs.map((p) => `<p${originalParagraphs.has(p) ? " data-guide-original" : ""}>${esc(p)}</p>`).join("")}</section>`).join("");
}
function guidePage(locale, guide) {
  ...
  const schemas = { "@context": "https://schema.org", "@graph": [{ "@type": "Article", headline: title, description, datePublished: EDITORIAL_UPDATED, dateModified: EDITORIAL_UPDATED, inLanguage: l.lang, mainEntityOfPage: { "@id": `${ORIGIN}${guideRoute}` }, author: { "@type": "Organization", name: c.team, url: `${ORIGIN}${route(locale, "/about")}` }, publisher: { "@type": "Organization", name: "Fursay", url: ORIGIN }, image: `${ORIGIN}/og-image.png` }, { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: l.home, item: `${ORIGIN}${route(locale, "/")}` }, { "@type": "ListItem", position: 2, name: l.guides, item: `${ORIGIN}${route(locale, "/guides")}` }, { "@type": "ListItem", position: 3, name: title, item: `${ORIGIN}${guideRoute}` }] }] };
  return `${head(locale, suffix, title, description, "article")}<body class="brand-surface editorial-page">${nav(locale, suffix)}<main class="editorial-shell"><article><header class="editorial-hero"><p class="brand-kicker">${guide.world === "koko" ? "Koko · English story practice" : "Nour · Chinese story practice"}</p><h1>${esc(title)}</h1><p class="editorial-lede">${esc(description)}</p><p class="editorial-byline" data-guide-editorial-byline><strong>${esc(c.team)}</strong> · ${c.published}: <time datetime="${EDITORIAL_UPDATED}">${EDITORIAL_UPDATED}</time> · ${c.updated}: <time datetime="${EDITORIAL_UPDATED}">${EDITORIAL_UPDATED}</time></p><p class="editorial-boundary">${esc(c.intro)}</p></header><nav class="editorial-toc" aria-label="${esc(c.contents)}"><strong>${esc(c.contents)}</strong><ol>${c.headings.slice(0, 7).map((item, i) => `<li><a href="#section-${i + 1}">${esc(item)}</a></li>`).join("")}</ol></nav><div class="editorial-body">${articleBody(locale, guide)}<section data-guide-sources><h2>${esc(c.headings[7])}</h2><p>${esc(c.sourceNote)}</p><ul class="source-list">${guide.sources.map(([name, url]) => `<li><a href="${esc(url)}" rel="noopener" target="_blank">${esc(name)}</a><p>${esc(c.methodLead)}</p></li>`).join("")}</ul><p data-guide-revision><strong>${c.updated}:</strong> ${esc(guide.details[locale].revision)}</p><p><a href="${route(locale, "/editorial-method")}">${esc(l.method)}</a> · <a href="${route(locale, "/contact")}#corrections">${esc(c.correction)}</a></p></section><aside class="editorial-next"><a class="brand-btn" href="${story}">${esc(c.related)}</a><a class="brand-btn brand-btn--secondary" href="${product}">${esc(c.sample)}</a></aside></div></article></main>${footer(locale)}<script type="application/ld+json">${JSON.stringify(schemas)}</script><script src="${SHARED_JS}"></script></body></html>`;
}
