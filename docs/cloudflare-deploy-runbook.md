...
- After a successful push deploy, the workflow skips automatic live smoke because the browser smoke step can time out after a successful deploy; live smoke remains a manual verification tool via `npm run smoke:live`.
...
- GitHub push-to-deploy should attach the real Analytics Engine report whenever the needed Cloudflare secrets are configured. Live smoke evidence is produced by the manual `npm run smoke:live` verification path.
