...
- Operations and product-validation page styles for `/creator-kit`, `/share-kit`, `/traffic-launch`, `/links`, `/deploy-readiness`, `/conversion-health`, and `/products` live in `/css/picture-world-tools-20260613-products1.css` so the main story pages do not load creator/share/deploy-only rules.
...
- `/products` renders the public product-interest landing page from `/products.json`; payment links remain disallowed while `paymentLinksAllowed=false`.
