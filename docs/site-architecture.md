...
- The generated editorial hubs, long-form guides, trust pages, and reviewer-facing utility navigation use `/css/editorial-20260802-v2.css`; their source data and templates live in `scripts/fursay-editorial-content.mjs`, `scripts/fursay-editorial-depth.mjs`, and `scripts/fursay-editorial-pages.mjs`.
...
- `/links`, `/creator-kit`, `/share-kit`, `/traffic-launch`, `/noor-sprint-status`, `/deploy-readiness`, `/conversion-health`, and `/monetization-roadmap` are utility surfaces.
...
- Owned products stay in interest-validation mode until the checkout gate has verified interest clicks, source-attributed sample traffic, price copy, disclosure copy, refund/support copy, checkout tracking, and a public sample-to-checkout route. Payment links are not allowed while `paymentLinksAllowed=false`.
